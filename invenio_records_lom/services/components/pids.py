...
class ExternalPIDsComponent(ServiceComponent):
        ...
        if not provider:  # In case we do not need to calculate it again
            ...
            if not provider:
                raise ValidationError(
                    message=_(
                        "Provider {provider_name} not found for PID " + "type {scheme}"
                    ).format(provider_name=provider_name, scheme=scheme),
                    field_name="pids",
                )
    ...
    def _validate_pid_schemes(self, pids):
        ...
        if unknown_pids:
            current_app.logger.error(
                "No configuration defined " f"for PIDs {unknown_pids}"
            )
            raise
    ...
    def _publish_managed(
        self, scheme, provider, is_required, draft_pid, record_pids, draft
    ):
    ...
    def _publish_unmanaged(
        ...
        if identifier_value:
            ...
            raise ValidationError(
                f"Value required for {scheme} PID.", field_name=f"pids.{scheme}"
            )
    ...
    def publish(self, identity, draft=None, record=None):
        ...
        for scheme, providers in pids_providers.items():
            ...
            provider = self.service.get_provider(scheme, pid_provider, pid_client)
            if not provider:
                continue
            ...
            if is_system_managed:
                self._publish_managed(
                    scheme, provider, is_required, draft_pid, record_pids, draft=draft
                )
            else:
