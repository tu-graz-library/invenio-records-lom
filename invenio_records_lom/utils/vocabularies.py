...
EnsureTypeTuple = namedtuple("EnsureTypeTuple", ["pid_type", "type_created"])
...
def ensure_type(
    ...
    if pid_type_in_db is None and pid_type is None:
        raise ValueError("couldn't automatically get `pid_type` when none was given")
    if (
    ...
    if pid_type_in_db is None:
        vocabulary_service.create_type(
            identity=system_identity, id=vocabulary_id, pid_type=pid_type, uow=uow
        )
        type_created = True
    else:
...
def import_vocabulary(
    ...
    soft_deleted_entry_ids = {
        ...
        for pid in PersistentIdentifier.query.filter_by(
            pid_type=pid_type, status=PIDStatus.DELETED
        )
    }
...
def undelete_vocabulary_entry(
    ...
    item._record.undelete()  # pylint: disable=protected-access
...
def delete_vocabulary_entry(
    ...
    uow.session.delete(entry_item._record.model)  # pylint: disable=protected-access
    # part of entry's data in `pidstore_pid`
    uow.session.delete(entry_item._record.pid)  # pylint: disable=protected-access
